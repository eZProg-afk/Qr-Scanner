...
public class QrGenerateRecyclerViewAdapter extends ListAdapter<QrGenerateItem, QrGenerateRecyclerViewAdapter.QrViewHolder> {
    ...
    public QrGenerateRecyclerViewAdapter() {
        super(DIFF_CALLBACK);
    }

    private static final DiffUtil.ItemCallback<QrGenerateItem> DIFF_CALLBACK = new DiffUtil.ItemCallback<QrGenerateItem>() {
        @Override
        public boolean areItemsTheSame(QrGenerateItem oldItem, QrGenerateItem newItem) {
            return oldItem.getId() == newItem.getId();
        }

        @Override
        public boolean areContentsTheSame(QrGenerateItem oldItem, QrGenerateItem newItem) {
            return oldItem.getResult().equals(newItem.getResult());
        }
    };

    public QrGenerateItem getQrItemAt(int position) {
    ...
    public void onBindViewHolder(@NonNull final QrViewHolder holder, int position) {
        final QrGenerateItem qrItem = getItem(position);
        ...
        if (type != null) {
            if (type.equals("youtube")) holder.imageView.setImageResource(R.drawable.ic_youtube);
            else if (type.equals("web")) holder.imageView.setImageResource(R.drawable.ic_web);
            ...
            else if (type.equals("product")) holder.imageView.setImageResource(R.drawable.ic_cart);
            else if (type.equals("email")) holder.imageView.setImageResource(R.drawable.ic_email);
            else if (type.equals("contact"))
                holder.imageView.setImageResource(R.drawable.ic_contact);
            else holder.imageView.setImageResource(R.drawable.simple_text);
        }
                ...
                builder.setItems(itemsForAlert, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        if (i == 0) {
                            ...
                            QrGenerateItemViewModel qrItemViewModel = new ViewModelProvider((ViewModelStoreOwner) context).get(QrGenerateItemViewModel.class);
    ...
    private void showFullImageWindow(QrGenerateRecyclerViewAdapter.QrViewHolder holder, QrGenerateItem qrItem) {
    ...
    public interface OnItemClickListener {
        void onItemClick(QrGenerateItem qrItem);
    }
